...
class Sandbox:
    ...
    def _connect_websocket(self, kernel_id: str, timeout: float = 30.0) -> None:
        ...
        try:
            ...
            raise ConnectionError("Failed to establish connection to code execution environment")
        ...
        if snapshot_id:
            ...
            if not snapshots:
                ...
                while time.time() - start_time < 90:  # 1.5 minute timeout
                    try:
                        response = requests.get(f"{jupyter_url}/api/kernels", timeout=5.0)
                        if response.status_code == 200:
                            jupyter_ready = True
                            break
                    except Exception:
                ...
                for language in LanguageSupport.get_supported_languages():
                    kernel_name = LanguageSupport.get_kernel_name(language)
                    print(f"Starting {language} kernel ({kernel_name})...", end="", flush=True)
                ...
                response = client._http_client.post(
                    ...
                    json={
                        ...
                        "metadata": {
                            ...
                            "languages": ",".join(LanguageSupport.get_supported_languages()),
                            "created_at": datetime.now().isoformat(),
                            "created_by": "sandbox"
                        }
                    }
